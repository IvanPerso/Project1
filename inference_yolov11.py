...
# ========================================
device = 'cuda:5'  # GPU
model = YOLO("runs-fire-smoke/yolov11-fire-smoke_epoch500/weights/best.pt").to(device)
...
def evaluate_model(model, image_paths, label_dir, iou_thresh=0.5): 
    TP = defaultdict(int)
    FP = defaultdict(int)
    FN = defaultdict(int)
    total_time = 0
    ...
    for img_path in tqdm(image_paths, desc="Evaluating"):
        ...
        start = time.time()
        results = model(str(img_path), conf=0.1, iou=0.45, device=device) ##Iou,conf 낮춰 recall 높이기 나중에에
        end = time.time()
